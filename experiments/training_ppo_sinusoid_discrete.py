...
pd_data_feeder.save_config(agent.logdir)  # PdDataFeederの設定をログディレクトリに保存
env.env.save_config(agent.logdir)  # 環境の設定をログディレクトリに保存
...
while True:  # 無限ループでエピソードを実行
    ...
    for index in memory.done_indices():
        ...
        if meanAverage.is_best(agent.epoch):
            agent.save_models('ppo_sinusoid')  # モデルを保存
