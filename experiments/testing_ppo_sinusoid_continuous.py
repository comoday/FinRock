...
while True:  # 無限ループでエピソードを実行
    # モデルによるアクションの予測（ランダムアクションの代わりにモデルを使用）
    action = agent.predict(np.expand_dims(state, axis=0), verbose=False)[0][:-1]  # 状態を次元拡張してモデルに入力

    # 環境にアクションを適用し、次の状態、報酬、終了フラグ、トランケートフラグ、情報を取得
    state, reward, terminated, truncated, info = env.step(action)  
    rewards += reward  # 報酬を累積
    pygameRender.render(info)  # 情報を描画

    # 環境が終了またはトランケートされた場合の処理
