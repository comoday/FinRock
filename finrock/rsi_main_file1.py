        ...
        async with pybotters.Client(apis=apis, base_url=base_url) as client:
            ...
            while True:
                ...
                for entry in kline:
                    ...
                    temp_df = temp_df.dropna(axis=1, how='all')  # 空のカラムを除外

                     # タイムスタンプが重複していないか確認
                    # if not data.empty and temp_df['timestamp'].iloc[0] in data['timestamp'].values:
                    #     logging.warning(f"Duplicate timestamp detected: {temp_df['timestamp'].iloc[0]}. Skipping this entry.")
                    #     continue
                ...
                # テクニカル指標を計算
                data_resampled['SMA5'] = talib.SMA(close_prices, timeperiod=5)
                ...
                data_resampled['PSAR'] = talib.SAR(high_prices, low_prices, acceleration=0.02, maximum=0.2)

                data_resampled['ADX'] = talib.ADX(high_prices, low_prices, close_prices, timeperiod=14)
                # data_resampled['BOP'] = talib.BOP(open_prices, high_prices, low_prices, close_prices)

                stoch_k, stoch_d = talib.STOCH(high_prices, low_prices, close_prices, fastk_period=5, slowk_period=3, slowk_matype=0, slowd_period=3, slowd_matype=0)
                ...
                if not data_resampled.empty:
                    logging.debug(f"Data resampled: {data_resampled.tail()}")   # デバック用ログ
